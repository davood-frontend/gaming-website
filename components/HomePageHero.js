        ...
        <>
            ...
            <Grid xs={12} sm={8} md={7} lg={8} sx={{ position: 'relative', gridTemplateRows: { xs: '250px 50px', md: '350px 50px', lg: '450px 50px' }, display: 'grid' }}>
                ...
                <Box sx={{ position: 'absolute', width: 1, height: 1, top: 0, right: 0, zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'start', paddingRight: 3 }}>
                    <Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8, mb: 1, cursor: 'pointer', color: !homePageHero.id ? grey[600] : 'white' }} onClick={() => homePageHandler('devilMayCry')} className='HomePageSliderController'>
                            <Circle sx={{fontSize:'15px'}}/>
                            <Typography sx={{ fontWeight: 500 }}>
                                دویل می کرای
                            </Typography>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, cursor: 'pointer', color: homePageHero.id ? grey[600] : 'white' }} onClick={() => homePageHandler('watchDogs')} className='HomePageSliderController'>
                            <Circle sx={{ fontSize: '15px' }} />
                            <Typography sx={{ fontWeight: 500 }}>
                                واچ داگز
                            </Typography>
