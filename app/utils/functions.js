...
//picking latest items (sort by the order thay are placed in array not by a specefic date)
...
//picking first items (sort by the order thay are placed in array not by a specefic date)
export const firstItemsPicker = (data) => {
    const res = data.slice(0, 7)
    return res
}

//formats numbers 3 by 3 using " , "
